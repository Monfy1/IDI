...
void MyGLWidget::iniEscena ()
{
  ...
  minC = glm::vec3(-10,0,-7);       //NOU
  maxC = glm::vec3(10,0,7);      //NOU
...
//  glViewport (0, 0, ample, alt);
  ...
  modelTransformIdent_1 ();
  // pintem cub
  glDrawArrays(GL_TRIANGLES, 0, 36);
  
  
  modelTransformIdent_2 ();
  glDrawArrays(GL_TRIANGLES, 0, 36);
  
  modelTransformIdent_3 ();
  glDrawArrays(GL_TRIANGLES, 0, 36);
...
void MyGLWidget::modelTransformModel ()
{
  ...
  TG = glm::translate(TG, glm::vec3(0,0,pasito));
...
void MyGLWidget::modelTransformIdent_1 ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(0,0,-6.9));
  TG = glm::scale(TG, glm::vec3(20, 2, 0.2));
...
void MyGLWidget::modelTransformIdent_2 ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(0,0,6.9));
  TG = glm::scale(TG, glm::vec3(20, 2, 0.2));
...
void MyGLWidget::modelTransformIdent_3 ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
...
void MyGLWidget::projectTransform ()
{
  ...
  if (novaC) Proj = glm::perspective (FOV_act, raw_act, zNear, zFar);
...
void MyGLWidget::viewTransform ()
{
  ...
  if(novaC) View = glm::lookAt(glm::vec3(0,18,0), glm::vec3(0,0,0),glm::vec3(-1,0,0));
...
{
  // Exercici 2-a): Cal fer el canvi de direcció de la pilota segons el rebot amb les parets
  // Fixeu-vos que el rebot amb una paret implica símplement el canvi de signe en una de
  // les components de la direcció de la pilota 
    if (-9 >= posPilota.x ) dirPilota.x = -dirPilota.x;
    if (-6 >= posPilota.z or posPilota.z >= 6) dirPilota.z = -dirPilota.z;
} 
...
void MyGLWidget::carregaShaders()
{
  ...
  esFLoc = glGetUniformLocation (program->programId(), "esF");
}

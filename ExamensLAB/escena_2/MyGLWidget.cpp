...
void MyGLWidget::initializeGL ()
{
  ...
  posPatr = glm::vec3(1, -0.375, 0);    //NOU
...
//  glViewport (0, 0, ample, alt);
  ...
  glBindVertexArray (VAO_Cow);   //NOU
  modelTransformVaca ();    //NOU
  glDrawArrays(GL_TRIANGLES, 0, cow.faces().size()*3);  //NOU
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(1,-0.375,0));   //NOU
  TG = glm::scale(TG, glm::vec3(escala, escala, escala));
  TG = glm::translate(TG, -centrePatr);

  //posPatr = TG * glm::vec4(centrePatr,1);   //malo
  //viewTransform();
  //update();
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  escala = 0.25/(maxy-miny);    //NOU
  centrePatr[0] = (minx+maxx)/2.0; centrePatr[1] = (miny+maxy)/2; centrePatr[2] = (minz+maxz)/2.0;   //NOU
}
...
void MyGLWidget::calculaCapsaModelVaca ()   //NOU
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
  ...
  centreCow[0] = (minx+maxx)/2.0; centreCow[1] = (miny+maxy)/2.0; centreCow[2] = minz;
}
void MyGLWidget::createBuffersVaca ()   //NOU
{
  // Carreguem el model de l'OBJ - Atenció! Abans de crear els buffers!
